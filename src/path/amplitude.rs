...
pub fn calculate_path_exafs(
    ...
    let effective_length = path.effective_length();
...
fn calculate_complex_amplitude(
    ...
    for i in 1..path.atom_sequence.len() - 1 {
        ...
        let scatterer_l = 0; // Use l=0 for s-wave scattering
        let scatterer_pot_index = scatterer.potential_type() as usize;
        
        if scatterer_l < scattering_results.phase_shifts[scatterer_pot_index].len() {
            // Add phase from the scatterer
            let scatterer_phase = scattering_results.phase_shifts[scatterer_pot_index][scatterer_l];
    ...
    complex_amplitude *= degeneracy_factor * s0_factor * mfp_damping * dw_damping * geometric_factor;
...
pub fn calculate_exafs_spectrum(
    ...
    for path in paths {
        let (amplitudes, phases) = calculate_path_exafs(
            path,
            structure,
            scattering_results,
            params,
