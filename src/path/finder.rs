...
impl Ord for RankedPath {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        other.path.importance.partial_cmp(&self.path.importance).unwrap_or(Ordering::Equal)
    }
}
...
impl PathFinder {
    ...
    fn precompute_neighbors(&mut self, max_distance: f64) {
        ...
        for i in 0..atom_count {
            ...
            for j in 0..atom_count {
                ...
                let pos_i = self.structure.atom(i).unwrap().position().clone();
                let pos_j = self.structure.atom(j).unwrap().position().clone();
    ...
    pub fn find_paths(&mut self) -> Vec<Path> {
        ...
        paths.sort_by(|a, b| b.importance.partial_cmp(&a.importance).unwrap_or(Ordering::Equal));
    ...
    fn extend_path(
        ...
        for &(next_atom, distance) in &self.neighbor_cache[last_atom] {
