...
use crate::utils::linear_algebra::{
    faer_to_ndarray, ndarray_to_faer, vector_norm, inner_product
};
use faer::{Mat, col};
use ndarray::Array2;
...
impl FmsSolver {
    ...
    fn solve_iterative(&self, fms_matrix: &Array2<Complex64>) -> Result<Array2<Complex64>> {
        ...
        if n > 128 {
            ...
            let column_results: Vec<Result<(usize, col::Col<Complex64>)>> = (0..n).into_par_iter().map(|col_idx| -> Result<(usize, col::Col<Complex64>)> {
    ...
    fn solve_cgs_for_column(&self, a: &Mat<Complex64>, col_idx: usize) -> Result<col::Col<Complex64>> {
        let n = a.nrows();
        ...
        for iter in 0..self.max_iterations {
            ...
            if rho.norm() < 1e-14 {
                return Err(FmsError::IterationFailed("CGS breakdown (rho ≈ 0)".to_string()));
            }
    ...
    fn solve_block_diagonal(&self, fms_matrix: &Array2<Complex64>) -> Result<Array2<Complex64>> {
        ...
        let block_size = {
            ...
            for size in 4..=36 {  // Try common block sizes (4, 9, 16, 25, 36)
                if n % size == 0 {
                    candidate_size = size;
                    break;
                }
        ...
        if block_size == 1 || n % block_size != 0 {
            return Err(FmsError::InvalidParameter(
                "Could not determine appropriate block size for block diagonal approximation".to_string()
            ));
        }
